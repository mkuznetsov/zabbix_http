...
idle(Event, State) ->
  gen_fsm:send_event_after(?COLLECTION_INTERVAL, post_events),
...
collecting(#event{}=Event, State) ->
  collecting([Event], State);
collecting([#event{}|_]=Events, State) ->
...
collecting(post_events, State=#state{cache = Tab, host=H,port = P}) ->
  % TODO: limit send list in case of it become too big
  EventList=[E||{_,E}<-ets:tab2list(Tab)],
...
idle({set_host_port,H,P}, _From, State) ->
  Reply = ok,
  {reply, Reply, idle, State#state{host=H,port = P}};
